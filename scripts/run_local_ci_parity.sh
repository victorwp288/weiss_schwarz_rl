...
if command -v uv >/dev/null 2>&1; then
  ...
  if [[ "$USE_SIM_EXTRA" == "1" ]]; then
    "${RUN[@]}" -m pip install --extra-index-url https://download.pytorch.org/whl/cu128 -e ".[dev,sim]"
  else
    "${RUN[@]}" -m pip install --extra-index-url https://download.pytorch.org/whl/cu128 -e ".[dev]"
  fi
fi
