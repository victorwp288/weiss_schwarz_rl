...
GROUP_LABEL="${BENCH_GROUP_LABEL:-vast_envelope_pass2_20260429_$(date +%H%M%S)}"
...
# Vast Benchmark Envelope Pass 2 - ${GROUP_LABEL}
...
run_case() {
  ...
  local actor_cap="$4"
  local max_envs_per_actor="$5"
  local batch_unrolls="$6"
  local label="${GROUP_LABEL}_w${width}_e${target_envs_per_gpu}_u${unroll_length}_a${actor_cap}_m${max_envs_per_actor}_b${batch_unrolls}"
  local exit_code=0
  ...
  .venv/bin/python - "$RESULTS_PATH" "$label" "$exit_code" "$width" "$target_envs_per_gpu" "$unroll_length" "$actor_cap" "$max_envs_per_actor" "$batch_unrolls" <<'PY'
...
max_envs_per_actor = sys.argv[8]
batch_unrolls = sys.argv[9]
...
row = (
    ...
    f"{max_envs_per_actor} | {batch_unrolls} | "
...
# Follow-up around the best first-pass point.
run_case 512 384 64 64 64 64
run_case 512 512 96 64 64 64
run_case 512 512 128 64 64 64
run_case 512 512 64 64 32 64

# Test model scaling above the requested 512 point without changing topology.
run_case 640 512 64 64 64 64
run_case 768 512 64 64 64 64

echo "Benchmark envelope pass 2 complete: ${RESULTS_PATH}"
