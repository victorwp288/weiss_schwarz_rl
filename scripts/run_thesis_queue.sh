...
if command -v uv >/dev/null 2>&1; then
  ...
  echo "Could not find uv or .venv/bin/python" >&2
  exit 1
fi
...
phase_matrix() {
  ...
  train_job \
    ...
    configs/presets/baselines/structured_acceptance_thesis_model_multideck_auto_gpu_noleague.yaml \
    "$ANCHOR_UPDATES" \
    "${IMPALA_TRAIN_OVERRIDES[@]}"
  thesis_job \
    "$MULTIDECK_RUN_LABEL" \
  ...
  train_job \
    ...
    configs/presets/baselines/structured_acceptance_thesis_model_reward_shaping_auto_gpu_noleague.yaml \
    "$ANCHOR_UPDATES" \
    "${IMPALA_TRAIN_OVERRIDES[@]}"
  thesis_job \
  ...
  train_job \
    ...
    configs/presets/baselines/norecurrence_impala_noleague.yaml \
    "$ANCHOR_UPDATES" \
    "${IMPALA_TRAIN_OVERRIDES[@]}"
  train_job \
