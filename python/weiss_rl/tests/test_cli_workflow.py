...
def test_parser_exposes_only_lean_thesis_workflow_commands() -> None:
    ...
    assert "train-b1-guided-seed" not in commands
    assert "train-main-guided-bootstrap" not in commands
    assert "guard-run" not in commands
    assert "guided-bootstrap-loop" not in commands
    assert "guarded-league-bootstrap" not in commands
