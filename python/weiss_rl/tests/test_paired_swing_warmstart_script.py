...
def test_paired_swing_warmstart_entrypoint_facade_reexports_cli_runtime_and_shared_helpers() -> None:
    from weiss_rl.training import (
        paired_swing_warmstart_cli,
        paired_swing_warmstart_entrypoint,
        paired_swing_warmstart_runtime,
        warmstart_replay_support,
    )

    assert paired_swing_warmstart_entrypoint._build_parser is (
        paired_swing_warmstart_cli.build_paired_swing_warmstart_parser
    )
    assert paired_swing_warmstart_entrypoint.run_paired_swing_warmstart is (
        paired_swing_warmstart_runtime.run_paired_swing_warmstart
    )
    assert paired_swing_warmstart_entrypoint._opponent_context_indices_for_episodes is (
        warmstart_replay_support._opponent_context_indices_for_episodes
    )
    assert paired_swing_warmstart_entrypoint._sample_episode_indices is warmstart_replay_support._sample_episode_indices


def test_paired_swing_warmstart_entrypoint_main_delegates_to_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    from weiss_rl.training import paired_swing_warmstart_entrypoint
    ...
    monkeypatch.setattr(paired_swing_warmstart_entrypoint, "run_paired_swing_warmstart", fake_run)
...
def test_paired_swing_warmstart_runtime_preserves_dataset_train_row_cli_error(
    ...
    from weiss_rl.training import paired_swing_warmstart_runtime
