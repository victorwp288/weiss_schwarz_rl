...
def test_run_final_eval_writes_matrix_exports_and_posterior_samples(tmp_path: Path) -> None:
    ...
    outcomes: dict[tuple[str, str, int, int], OutcomeToken] = {
        ...
        ("policy_alpha", "policy_beta", 0, 0): "L",
        ("policy_alpha", "policy_beta", 0, 1): "L",
        ("policy_alpha", "policy_beta", 1, 0): "L",
    ...
    assert stop_reason_rows == [
        ...
        ["policy_alpha", "decisive", "precision"],
    ]
    ...
    decisive_summary = json.loads(
        (output_dir / "matchups" / "01_policy_alpha__vs__00_policy_beta" / "matchup_summary.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert decisive_summary["stop_reason"] == "decisive"
