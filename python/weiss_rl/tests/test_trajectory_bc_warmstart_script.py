...
def test_trajectory_bc_warmstart_entrypoint_facade_reexports_cli_runtime_and_helpers() -> None:
    from weiss_rl.training import (
        trajectory_bc_warmstart_cli,
        trajectory_bc_warmstart_entrypoint,
        trajectory_bc_warmstart_runtime,
    )

    assert trajectory_bc_warmstart_entrypoint._build_parser is (
        trajectory_bc_warmstart_cli.build_trajectory_bc_warmstart_parser
    )
    assert trajectory_bc_warmstart_entrypoint.run_trajectory_bc_warmstart is (
        trajectory_bc_warmstart_runtime.run_trajectory_bc_warmstart
    )
    assert trajectory_bc_warmstart_entrypoint._initial_hidden_state is (
        trajectory_bc_warmstart_runtime._initial_hidden_state
    )
    assert trajectory_bc_warmstart_entrypoint._publish_trajectory_bc_snapshot is (
        trajectory_bc_warmstart_runtime._publish_trajectory_bc_snapshot
    )


def test_trajectory_bc_warmstart_entrypoint_main_delegates_to_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    from weiss_rl.training import trajectory_bc_warmstart_entrypoint
    ...
    monkeypatch.setattr(trajectory_bc_warmstart_entrypoint, "run_trajectory_bc_warmstart", fake_run)
...
def test_trajectory_bc_warmstart_runtime_preserves_dataset_train_row_cli_error(
    ...
    from weiss_rl.training import trajectory_bc_warmstart_runtime
