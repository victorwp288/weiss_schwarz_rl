...
from weiss_rl.runtime.components.rewards.reward_flow import reward_flow_stage_payload
from weiss_rl.runtime.components.rewards.reward_shaping_plan import collector_reward_shaping_plan_payload

COMPONENT_NAMES = ("terminal", "damage", "level", "board", "no_progress")


@dataclass(frozen=True, slots=True)
class RewardComponentProbeCheck:
    component: str
    question: str
    expected_signal: str
...
REWARD_COMPONENT_PROBE_PLAN: tuple[RewardComponentProbeCheck, ...] = (
    RewardComponentProbeCheck(
        component="terminal",
    ...
    RewardComponentProbeCheck(
        component="damage",
        question="Is damage shaping active at the configured scale?",
        expected_signal="small intermediate values when damage changes before terminal outcome",
    ),
    RewardComponentProbeCheck(
        component="level",
        question="Is level-progress shaping active without dominating terminal reward?",
        expected_signal="small intermediate values tied to level changes",
    ),
    RewardComponentProbeCheck(
        component="board",
        question="Is board-state shaping active only when configured?",
        expected_signal="zero or small values depending on the stack reward config",
    ),
    RewardComponentProbeCheck(
        component="no_progress",
        question="Is no-progress pressure visible without hiding terminal outcome?",
        expected_signal="small penalties on configured stalled transitions",
    ),
)


def reward_component_probe_plan_payload() -> list[dict[str, str]]:
    return [check.as_payload() for check in REWARD_COMPONENT_PROBE_PLAN]
...
def summarize_reward_samples(
    *,
    ...
    return {
        "reward_flow": reward_flow_stage_payload(),
        "probe_plan": reward_component_probe_plan_payload(),
        "collector_reward_shaping_plan": collector_reward_shaping_plan_payload(),
