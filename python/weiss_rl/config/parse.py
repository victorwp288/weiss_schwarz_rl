...
def _resolve_legacy_config_path(stack_file: Path) -> Path:
...
def _require_auto_or_int(value: Any, *, field_name: str, minimum: int = 0) -> str:
...
def _parse_training_config(body: dict[str, Any]) -> TrainingConfig:
    ...
    if raw_b1_distill_enabled and (raw_b1_distill_teacher_surface != "raw_s0" or raw_b1_distill_student_surface != "raw_s0"):
        raise ValueError("training.raw_b1_distill currently supports only teacher_surface=raw_s0 and student_surface=raw_s0")
    ...
    if (
        ...
        raise ValueError(
            "training.counterfactual_positive.end_updates must be >= "
            "training.counterfactual_positive.start_updates"
        )
    ...
    main_residual_initial_state_path = str(
        main_residual_policy.get("initial_residual_state_path", "") or ""
    ).strip()
    ...
    for index, entry in enumerate(residual_opponent_policy_entries):
        ...
        if bias_scale < 0.0:
            raise ValueError(
                f"training.residual_opponent_policies[{index}].public_heuristic_bias_scale must be >= 0.0"
            )
...
def _parse_league_config(body: dict[str, Any]) -> LeagueConfig:
    ...
    for anchor_name, score in eval_gate_min_anchor_scores_raw.items():
        ...
        if not math.isfinite(score_f) or score_f < 0.0 or score_f > 1.0:
            raise ValueError(
                "league.warmup.eval_gate_min_anchor_scores values must be finite probabilities in [0, 1]"
            )
...
def _parse_evaluation_config(body: dict[str, Any]) -> EvaluationConfig:
    ...
    invalid_anchor_weights = {
        key: value
        for key, value in periodic_dev_eval_anchor_weights.items()
