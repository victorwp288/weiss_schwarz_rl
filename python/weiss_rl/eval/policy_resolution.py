"""Policy and snapshot-registry path resolution for canonical eval."""
...
from weiss_rl.eval.policy_set import NO_LEAGUE_POLICY_ID
from weiss_rl.league.registry import SnapshotMeta, SnapshotRegistry

LEGACY_B1_POLICY_ID = "b1_noleague_baseline"
...
def resolve_snapshot_registry_run_dir(
    *,
    ...
    if canonical_search_root is not None:
        ...
        canonical_common_search_root = common_search_root([resolved_run_dir, canonical_candidate])
...
def snapshot_registry_resolution_snapshots(
    *,
    ...
    def append_snapshot(policy_id: str) -> None:
    ...
    for policy_id in policy_ids:
        append_snapshot(policy_id)
    if NO_LEAGUE_POLICY_ID in policy_ids:
        append_snapshot(LEGACY_B1_POLICY_ID)
