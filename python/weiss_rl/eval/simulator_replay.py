...
from weiss_rl.envs.pool_factory import build_env_config_from_stack
from weiss_rl.eval.harness import ReplaySampleResult, ScheduledGame
...
class SimulatorReplayRecorder:
    ...
    def maybe_start(
        ...
        *,
        ...
        if not self._should_capture(scheduled_game=scheduled_game):
            return None
        raw_dir = self.artifact_layout.replays_raw_dir / self._sample_dir_name(scheduled_game=scheduled_game)
    ...
    def finish(
        ...
        *,
        ...
        raw_replay_path = self._discover_raw_replay_path(capture=capture)
    ...
    def _should_capture(self, *, scheduled_game: ScheduledGame) -> bool:
        if self.capture_rate <= 0.0:
            return False
        if self._capture_count >= self.capture_limit:
            return False
        ...
        return (capture_u64 / _U64_DENOMINATOR) < self.capture_rate

    def _visibility_mode(self) -> str:
    ...
    def _sample_dir_name(self, *, scheduled_game: ScheduledGame) -> str:
    ...
    def _discover_raw_replay_path(self, *, capture: ReplayCaptureState) -> Path | None:
        after_paths = set(capture.raw_dir.glob("*.wsr"))
        new_paths = sorted(after_paths - capture.before_raw_paths)
        if len(new_paths) == 1:
            return new_paths[0]
