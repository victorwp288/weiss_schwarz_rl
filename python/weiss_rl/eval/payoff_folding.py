...
def _validate_pair_records(records: Sequence[EvalGameRecord]) -> tuple[EvalGameRecord, EvalGameRecord]:
    ...
    if len({paired_seed_group_key(record) for record in records}) != 1:
...
def _paired_seed_group_sort_key(group_key: PairedSeedGroupKey) -> tuple[str, str, str, str, str, int]:
    focal_policy_id, opponent_policy_id, config_hash256, spec_hash256, run_id256, pair_index = group_key
    return (
        focal_policy_id,
        opponent_policy_id,
        config_hash256,
