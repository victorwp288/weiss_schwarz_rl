...
def build_training_workflow_plan_for_request(request: TrainingWorkflowRequest) -> TrainingWorkflowPlan | None:
    ...
    if request.command == "train-b1-guided-seed":
        return build_b1_guided_seed_training_workflow_plan_for_request(request)

    if request.command == "train-main":
        return build_main_training_workflow_plan_for_request(request)

    if request.command == "train-main-guided-bootstrap":
        return build_main_guided_bootstrap_training_workflow_plan_for_request(request)
