...
from weiss_rl.workflows.training_profiles import B1_GUIDED_SEED_STACK_CONFIG, B1_STACK_CONFIG, TRAIN_PROFILES
...
def build_b1_guided_seed_training_workflow_plan_for_request(request: TrainingWorkflowRequest) -> TrainingWorkflowPlan:
    args = request.args
    profile = TRAIN_PROFILES[str(args.profile)]
    return TrainingWorkflowPlan(
        plan_name=str(args.run_label),
...
def build_b1_guided_seed_training_workflow_plan(*, args: argparse.Namespace, python_exe: str) -> TrainingWorkflowPlan:
    return build_b1_guided_seed_training_workflow_plan_for_request(
        training_workflow_request(args=args, repo_root=Path(), python_exe=python_exe)
    )
...
__all__ = [
    ...
    "build_b1_guided_seed_training_workflow_plan_for_request",
