...
def _dev_eval_records_from_training_log(run_dir: Path) -> list[dict[str, Any]]:
    payload = _json_or_none(run_dir / "training" / "logs" / "periodic_dev_eval_summaries.json")
    if payload is None:
        return []
...
def _dev_eval_records_from_eval_dirs(run_dir: Path) -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
...
def load_dev_eval_records(run_dir: Path) -> list[dict[str, Any]]:
    records = _dev_eval_records_from_training_log(run_dir)
    return records if records else _dev_eval_records_from_eval_dirs(run_dir)


def _best_and_latest(records: list[dict[str, Any]]) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    if not records:
        return None, None
...
def _score_range(values: list[float]) -> dict[str, float | int]:
    return {
        "count": len(values),
        "min": min(values),
        "max": max(values),
...
def _numeric_at_path(payload: dict[str, Any], path: tuple[str, ...]) -> float | None:
    cursor: Any = payload
    for key in path:
...
def load_learning_progress_metrics(run_dir: Path) -> dict[str, float]:
    payload = _json_or_none(run_dir / "diagnostics" / "learning_progress_summary.json")
    if payload is None:
        return {}
    metrics: dict[str, float] = {}
    for metric_name, path in DIAGNOSTIC_METRIC_PATHS.items():
        value = _numeric_at_path(payload, path)
        if value is not None:
            metrics[metric_name] = value
    return metrics


def _family_alignment_by_name(scope_payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
...
def _policy_alignment_metric_prefix(anchor_name: str) -> str | None:
...
def load_policy_alignment_metrics(run_dir: Path, *, update_count: int | None) -> dict[str, float]:
    if update_count is None:
        return {}
    summary = _json_or_none(run_dir / "eval" / "dev_eval" / f"update_{int(update_count)}" / "summary.json")
    if summary is None:
        return {}
    ...
    for anchor_name, anchor_payload in anchors.items():
        if not isinstance(anchor_name, str) or not isinstance(anchor_payload, dict):
            continue
        prefix = _policy_alignment_metric_prefix(anchor_name)
        if prefix is None:
            continue
        ...
        for source_name, metric_name in (
            ...
            value = _numeric_at_path(scope_payload, (source_name,))
            if value is not None:
                metrics[f"{prefix}_{metric_name}"] = value

        family_summaries = _family_alignment_by_name(scope_payload)
        for family_name in POLICY_ALIGNMENT_FAMILIES:
            ...
            for source_name, metric_name in (
                ...
                value = _numeric_at_path(family_payload, (source_name,))
                if value is not None:
                    metrics[f"{prefix}_{family_name}_{metric_name}"] = value
            margin_mean = _numeric_at_path(
                family_payload,
...
def _named_score_range(values: list[tuple[str, float]]) -> dict[str, float | int | str]:
    ...
    summary = _score_range(numeric_values)
...
def build_run_learning_comparison(
    ...
    *,
    ...
    for run_dir in run_dirs:
        ...
        best, latest = _best_and_latest(records)
        runs.append(
            {
                "run_dir": str(run_dir),
    ...
    for update_count, entries in sorted(update_values.items()):
        ...
        aggregate_summary = _score_range(aggregate_values)
        anchor_names = sorted(
        ...
        for anchor_name in anchor_names:
            ...
            if len(values) >= 2:
                anchor_summaries[anchor_name] = _score_range(values)
    ...
    diagnostic_metric_ranges = {
        metric_name: _named_score_range(values)
