#!/usr/bin/env python3
"""Run guarded short-segment league bootstrap with B2/B3/B4 gates."""
...
def _uses_seed_snapshot_opponents(values: list[str] | None) -> bool:
    return any(str(value).strip().startswith("seed_") for value in values or [])


def _has_seed_snapshot_pool_override(values: list[str] | None) -> bool:
    prefixes = (
...
def _validate_seed_snapshot_policy(args: argparse.Namespace) -> None:
    opponent_args = [*(args.required_anchor or []), *(args.confirm_opponent or [])]
    if (
        _uses_seed_snapshot_opponents(opponent_args)
        and not bool(args.use_stack_seed_snapshot_policy)
        and not _has_seed_snapshot_pool_override(args.override)
    ):
        raise SystemExit(
...
def main() -> None:
    args = parse_args()
    _validate_seed_snapshot_policy(args)
    ...
    config = GuardedLeagueBootstrapConfig(
        ...
        multiobjective_reference_summary_jsons=multiobjective_reference_summary_jsons,
