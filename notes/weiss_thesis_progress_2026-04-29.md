...
## 2026-04-30 - Main v14b gentle ramp continuation from v14 u160

Purpose:

- Test whether the v14 u160 checkpoint could be gently improved without falling out of the B1-safe basin.
- Continue from the best v14 main candidate instead of restarting from scratch.
- Keep the confirmed v14 fixes:
  - actor/learner heuristic-bias parity
  - non-decaying exact B1 BC
  - non-decaying family B1 BC
  - low CF pressure
  - zero damage shaping
...
- `configs/main_impala_league_server_v14b_u160_gentle_ramp.yaml`
...
- LR:
  - `7.5e-6 -> 5e-6`
- RL pressure:
  - `policy_loss_coef: 0.50 -> 0.35`
- B1 baseline mix:
  - `0.45 -> 0.42`
- base public heuristic mix:
  - `0.03 -> 0.04`
- heuristic public variant mix:
  - `0.08 -> 0.12`
- unchanged:
  - actor bias follows learner via `public_heuristic_actor_logit_bias_scale: -1.0`
  - exact B1 BC `0.25` constant
  - family B1 BC `0.40` constant
  - CF `0.10` constant
  - damage reward `0.0`
  - champions/hard negatives/warmup snapshots `0.0`
  - warmup first updates `1000`

Runtime sanity:

- u162:
  - actor bias active `2.0`
  - family BC active `0.40`
  - B1 mix `0.42`
  - variant mix `0.12`
  - policy loss coef `0.35`
  - `vtrace_rho_p99` about `132`
- u240:
  - `vtrace_rho_p99` about `86`
  - exact BC loss about `0.489`
  - family BC loss about `4.013`
- u320:
  - `vtrace_rho_p99` about `14`
  - exact BC loss about `0.489`
  - family BC loss about `3.736`

Results:

- Manual confirm64 B1-only at u180:
  - B1 `0.4921875`
- Automatic 16-pair dev eval at u240:
  - B0 `1.000`
  - B1 `0.500`
  ...
  - B3 `0.750`
  - B4 `0.96875`
  - aggregate `0.6958333333333333`
- Manual confirm64 B1-only at u240:
  - B1 `0.484375`
- Manual confirm64 B1-only at u320:
  - B1 `0.4140625`

Interpretation:

- v14b did not produce a better main candidate than v14 u160.
- The off-policy instability stayed fixed, but B1 preservation still decayed under even a gentle RL/variant continuation.
- u320 should not be used.
- u240 is also below the B1 safety line on confirm64, so a full matrix is not worth spending time on.
...
- If continuing league experiments, do not simply run v14b longer.
- Either:
