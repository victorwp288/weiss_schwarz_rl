...
def test_negative_logits_fill_value_uses_dtype_min_for_float_masks() -> None:
    fill_value = negative_logits_fill_value(torch.float16)

    assert fill_value == float(torch.finfo(torch.float16).min)
    assert torch.isfinite(torch.tensor(fill_value, dtype=torch.float32))
