...
import pytest
import torch
import weiss_rl.training.replay_data.paired_outcome_preference_dataset as paired_outcome_preference_dataset
import weiss_rl.training.replay_data.paired_outcome_preference_replay as paired_outcome_preference_replay
...
def test_paired_outcome_preference_replay_is_not_a_training_root_alias() -> None:
    import weiss_rl.training as training

    assert not hasattr(training, "paired_auxiliary_replay")
    assert not hasattr(training, "paired_outcome_preference_dataset")
    assert not hasattr(training, "paired_outcome_preference_replay")
...
def test_paired_outcome_preference_replay_does_not_reexport_dataset_helpers() -> None:
    assert paired_outcome_preference_replay.__all__ == [
        "PairedOutcomePreferenceReplayState",
        "maybe_run_paired_outcome_preference_replay",
    ]
    assert not hasattr(paired_outcome_preference_replay, "paired_outcome_preference_complete_pair_count")
    assert callable(paired_outcome_preference_dataset.paired_outcome_preference_complete_pair_count)
    assert (
